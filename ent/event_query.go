...
func (_q *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := _q.withJourney; query != nil {
		if err := _q.loadJourney(ctx, query, nodes, nil,
			func(n *Event, e *Journey) { n.Edges.Journey = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *EventQuery) loadJourney(ctx context.Context, query *JourneyQuery, nodes []*Event, init func(*Event), assign func(*Event, *Journey)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Event)
	for i := range nodes {
